...
describe('/share-project', () => {
  describe('ShareProject#BEFORE', () => {
    it('should require projectid', async () => {
      const req = TestHelper.createRequest('/share-project?projectid=', 'GET')
      let errorMessage
      try {
    ...
    it('should require own projectid', async () => {
      ...
      const req = TestHelper.createRequest(`/share-project?projectid=${project1.projectid}`, 'GET')
    ...
    it('should reject shared project', async () => {
      ...
      const req = TestHelper.createRequest(`/share-project?projectid=${project.projectid}`, 'GET')
  ...
  describe('ShareProject#POST', () => {
    it('should apply after authorization', async () => {
      const user = await TestHelper.createUser()
      const project = await TestHelper.createProject(user)
      const req = TestHelper.createRequest(`/share-project?projectid=${project.projectid}`, 'POST')
      req.account = user.account
      req.session = user.session
      const res = TestHelper.createResponse()
      res.end = async (str) => {
