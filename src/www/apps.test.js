...
describe('/apps', () => {
  describe('Apps#BEFORE', () => {
    it('should bind apps to req', async () => {
      ...
      const req = TestHelper.createRequest(`/apps`, 'GET')
  ...
  describe('Apps#GET', () => {
    it('should limit apps to one page', async () => {
      ...
      for (let i = 0, len = global.pageSize + 1; i < len; i++) {
        await TestHelper.createApp(user, { url: 'https://something', stripeid: 'stripe_1', application_fee: '0.05' })
        await TestHelper.wait()
      }
      const req = TestHelper.createRequest('/apps', 'GET')
      req.account = user.account
    ...
    it('should enforce specified offset', async () => {
      ...
      const req = TestHelper.createRequest(`/apps?offset=${offset}`, 'GET')
      req.account = user.account
      req.session = user.session
      const res = TestHelper.createResponse()
      res.end = async (str) => {
