...
async function beforeRequest(req) {
  ...
  if (server.projectid) {
    req.query.projectid = server.projectid
     const project = await global.api.user.userappstore.Project._get(req)
     if (req.account.accountid !== project.accountid) {
       throw new Error('invalid-account')
     }
  } else if (server.ownerid !== req.account.accountid) {
    throw new Error('invalid-account')
  }
  ...
  req.data = { profiles: eligible, allProfiles: profiles, server }
}
...
async function submitForm(req, res) {
  ...
  try {
    await dashboardServer.post(`/api/application-server/create-owner?serverid=${req.query.serverid}`, req.body, req.account.accountid, req.session.sessionid)
    res.statusCode = 302
