...
describe('/add-items', () => {
  describe('AddItems#BEFORE', () => {
    it('should require collectionid', async () => {
      const req = TestHelper.createRequest('/add-items?collectionid=', 'GET')
      let errorMessage
      try {
    ...
    it('should require own collectionid', async () => {
      ...
      const req = TestHelper.createRequest(`/add-items?collectionid=${collection1.collectionid}`, 'GET')
    ...
    it('should bind collection to req', async () => {
      ...
      const req = TestHelper.createRequest(`/add-items?collectionid=${collection.collectionid}`, 'GET')
  ...
  describe('AddItems#GET', () => {
    it('should present form for each install not in collection', async () => {
      const user = await TestHelper.createUser()
      const collection = await TestHelper.createCollection(user)
      ...
      res.end = async (str) => {
        ...
        assert.strictEqual(doc.getElementById(install3.installid).tag, 'li')
      }
      await req.route.api.before(req)
  ...
  describe('AddItems#POST', () => {
    it('should reject invalid installid', async () => {
      const user = await TestHelper.createUser()
      const collection = await TestHelper.createCollection(user)
      const req = TestHelper.createRequest(`/add-items?collectionid=${collection.collectionid}`, 'POST')
      req.account = user.account
      req.session = user.session
      req.body = {
        installid: 'invalid'
    ...
    it('should add items after authorization', async () => {
      const user = await TestHelper.createUser()
      const collection = await TestHelper.createCollection(user)
